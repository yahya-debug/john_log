...
export async function upsertHourlyCounts(entries: ValidatedLog[], client: SqlClient = db.$client) {
    ...
    const counts = new Map<string, { hour: string; service: string; level: string; count: number }>();
    for (const e of entries) {
        ...
        else counts.set(key, { hour, service: e.service, level: e.level, count: 1 });
    }
...
// the wrong thing.
export function buildQueryLogsQuery(conditions: SQL | undefined, limit: number) {
...
export function buildAggregateLogsQuery(conditions: SQL | undefined, bucket_size: string, group?: "service" | "level" | null | undefined) {
...
export async function aggregateFromRollup(
    ...
    if (filters.service) conditions.push(eq(logsHourlyCounts.service, filters.service));
    if (filters.level) conditions.push(eq(logsHourlyCounts.level, filters.level));
    ...
    const bucketing = (
        bucket === "1d"
            ? sql<string>`date_bin('1 day'::interval, ${logsHourlyCounts.hour}, TIMESTAMPTZ '2000-01-01')`
            : sql<string>`${logsHourlyCounts.hour}`
    ).as('start');
    ...
    const baseQuery = db.select({
        start: bucketing,
        group: groupField,
        count: sql<number>`SUM(${logsHourlyCounts.count})::int`,

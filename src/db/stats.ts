...
async function execute<T extends Record<string, unknown>>(query: Parameters<typeof db.execute>[0]): Promise<T[]> {
    return db.execute<T>(query) as Promise<T[]>;
}
...
export async function getStats(): Promise<Stats> {
    const [partitionRows, levelRows, serviceRows, rangeRows, rateRows] = await Promise.all([
        execute<{ name: string; bound: string | null; rows_estimate: number; size_bytes: number }>(sql`
            SELECT
                c.relname AS name,
        ...
        execute<{ last_1m: number; last_5m: number }>(sql`
            SELECT

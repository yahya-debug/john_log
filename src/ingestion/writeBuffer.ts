...
export type PushResult = { admitted: true } | { admitted: false; retryAfterSec: number };
...
export async function pushLogs(entries: ValidatedLog[]): Promise<PushResult> {
...
function scheduleFlush(): void {
    if (inFlight.size >= MAX_CONCURRENT_FLUSHES) return;

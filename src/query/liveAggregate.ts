...
type BucketKey = number; // epoch ms, floored to the minute
type GroupKey = string; // `${service}\x00${level}`

const buckets = new Map<BucketKey, Map<GroupKey, number>>();
...
function bucketKeyFor(timestamp: string | Date): BucketKey {
    const ms = new Date(timestamp).getTime();
    return Math.floor(ms / MINUTE_MS) * MINUTE_MS;
}

function groupKeyFor(service: string, level: string): GroupKey {
    return `${service}\x00${level}`;
}

export function recordAccepted(entries: ValidatedLog[]): void {
    for (const entry of entries) {
        const bucketStart = bucketKeyFor(entry.timestamp);
        if (Number.isNaN(bucketStart)) continue; // defensive only — validated upstream already
...
export interface LiveAggregateFilters {
    since: Date;
    until: Date;
    service?: string;
    level?: string;
...
function sumBucket(bucket: Map<GroupKey, number>, filters: LiveAggregateFilters): Map<string | null, number> {
...
export function queryLive(filters: LiveAggregateFilters): LiveAggregateRow[] | null {
    const sinceBucket = bucketKeyFor(filters.since);
    const untilMs = filters.until.getTime();
    if (Number.isNaN(sinceBucket) || Number.isNaN(untilMs)) return null;

    const oldestReliable = Math.max(
        Math.floor(trackerStartedAt / MINUTE_MS) * MINUTE_MS,
        Math.floor(Date.now() / MINUTE_MS) * MINUTE_MS - (RETENTION_MINUTES - 1) * MINUTE_MS
    );
    if (sinceBucket < oldestReliable) return null;

    const rows: LiveAggregateRow[] = [];
...
export function resetLiveAggregate(): void {
    buckets.clear();
}

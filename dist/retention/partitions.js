 ...
 */
export async function ensureFuturePartitions(daysAhead) {
    const created = [];
...
async function listDatedPartitions() {
...
export async function dropOldPartitions(retentionDays) {
 ...
 */
export async function backfillPartitionForDate(date) {
    const name = partitionName(date);
    const { from, to } = dayBounds(date);
    return await db.transaction(async (tx) => {
        ...
        async function moveMatchingRows() {
        ...
        const existsRows = await tx.execute(sql `SELECT EXISTS (SELECT 1 FROM pg_class WHERE relname = ${name}) AS exists`);
 ...
 * outright instead.
 */
export async function reconcileDefaultDate(date, retentionDays) {
    const cutoff = new Date();
    cutoff.setUTCDate(cutoff.getUTCDate() - retentionDays);
    if (date < cutoff) {
        const { from, to } = dayBounds(date);
...
export async function reconcileDefaultPartition(retentionDays) {
    const rows = await db.execute(sql `SELECT DISTINCT date_trunc('day', timestamp)::date AS day FROM logs_default`);
    let deleted = 0;
    const backfilled = [];
    for (const { day } of rows) {
        ...
        if (result.action === "deleted")
            ...
            backfilled.push(partitionName(date));
    }
